...
def explore_kmeans_distributions_and_centroids(model: KMeans, X: pd.DataFrame,
    ...
    for current_cluster_index in range(kmeans_model.n_clusters):
        ...
        if lows_pct > 70.0 or highs_pct > 70.0:
            logging.info(f"Cluster [{current_cluster_index}]. Support [{support:.4f}]. Lows [{lows_pct:.4f}]. Highs [{highs_pct:.4f}]. (*)")
        ...
        for current_feature in numeric_feature_names:
            ...
            if sum(p_values_less_than_0_05) == len(p_values_less_than_0_05):
                logging.info(f"\tIdentified by feature [{current_feature}]. Range [{low_ci:.3f} - {high_ci:.3f}].")

    logging.info(f"Support of clusters associated with a level of experience: {sum(supports_of_clusters_associated_with_an_expertise_level):.4f}.")
...
if __name__ == '__main__':
    ...
    assert os.path.isfile(numeric_features_path), "No numeric features file found at {}".format(features_path)
    ...
    numeric_features = [feature.rstrip() for feature in open(numeric_features_path, mode='r').readlines()]
